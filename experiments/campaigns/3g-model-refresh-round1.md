...
| H4 | Keep the same hybgzs `#gpt group` credential and `gpt-5.4-mini` route as `H3`, but lower concurrency to `EXP_N_PROC=1` for the recommended serial follow-up on the same cached `quetra-pop-5` setup | completed | `20260429-3g-model-refresh-hybgzs-gpt54mini-n1-r1` | `ABRBench-3G = 85.3480`; serial full run completed with 3 recovered first-attempt `HTTP 504` errors and no `HTTP 429` or `BrokenPipeError`, clearing the earlier startup-capacity failure mode but still landing `-1.6047` below historical `QUETRA pop5 = 86.9527` and `-2.2264` below `H1` |
...
19. The recommended serial follow-up confirms that the earlier `H3` failure was at least partly a concurrency / provider-capacity problem, not just a model-quality collapse.
    ...
    - unlike `H3`, it completed a full serial run and produced `ABRBench-3G = 85.3480`
    - transport was not perfect but was recoverable: 3 first-attempt `HTTP 504` errors, with no `HTTP 429` or `BrokenPipeError`
