...
| H2 | Use the hybgzs `#gpt group` credential with `gpt-5.4`, keeping `QUETRA + pop5 + gen10` but raising concurrency to `EXP_N_PROC=4` | running | `20260429-3g-model-refresh-hybgzs-gpt54-n4-r1` | min API probe returned `OK`; launched on heyun with cached `quetra-pop-5` seed and entered `OP: e1 [1/5]` without early 401/429/API errors |
