...
| P2-grok2api-rmpc_predictor-close-r1 | Full cache-backed `rmpc_predictor` rerun with `LLM_API_CONNECTION_CLOSE=1` on the same grok2api branch | completed | `20260428-3g-advanced-rmpc-predictor-grok2api-close-r1` | `ABRBench-3G avg = 86.3736`; completed full pipeline with `BrokenPipeError=0`, one HTTP 429 retry, and 18 invalid offspring; much cleaner and stronger than keep-alive `P2`, but still below historical `QUETRA pop5 = 86.9527` |
...
- `P2-grok2api-rmpc_predictor-close-r1` is the clean quality-bearing `rmpc_predictor` result for this round. It improved the keep-alive full run by `+4.0266` points (`82.3470` -> `86.3736`) and removed the repeated BrokenPipe failure mode (`107` -> `0`). It also beat the same-campaign DeepSeek `quetra` control (`83.9349`) and the completed `virtual_sensor` run (`85.8634`), but it still missed historical `QUETRA pop5 = 86.9527` by `-0.5791` and A1 by `-2.4961`.
...
2. Treat `rmpc_predictor` as the stronger structured-seed signal from this round, but not yet a new baseline because it still trails historical `QUETRA pop5`.
