...
| 3G Advanced Architectures Round 1 | 把 Gemini 提议里的 `virtual sensor` 和 `MPC predictor` 各自压成最小可跑 seed family，先验证结构化架构在 3G 上是否比自由逻辑 seed 更有信号 | completed | `rmpc_predictor` with grok2api connection-close reached 86.4 with 0 BrokenPipe, beating same-campaign controls but still below historical `QUETRA pop5` 87.0 | [tracker](3g-advanced-architectures-round1.md) |
...
| grok2api Connection Close Probe | 验证 grok2api `BrokenPipeError` 是否来自复用 keep-alive 连接，并测试 opt-in `Connection: close` 开关 | completed | full `rmpc_predictor` run finished at 86.4 with `BrokenPipeError=0`, versus keep-alive run's 82.3 and 107 BrokenPipe retries | [tracker](grok2api-connection-close-probe.md) |
