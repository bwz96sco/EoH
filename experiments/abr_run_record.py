"""Shared ABR experiment record code.

Dataclasses, constants, and discovery/formatting helpers extracted from the
original ``update_experiment_tracker.py`` so that other scripts
(``generate_run_report.py``, analysis tools, etc.) can reuse them without
pulling in the tracker's CLI or rendering logic.
"""
